DROP PROCEDURE Paso4_ViajesAlternativos;
CREATE PROCEDURE Paso4_ViajesAlternativos
...
-------------------------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------------
DROP PROCEDURE Paso5_ViajesPropiosEnCorredor;
CREATE PROCEDURE Paso4_ViajesPropiosEnCorredor
 @Database NVARCHAR(15), -- Parámetro de entrada para la base de datos
 @IdLinea NVARCHAR(10),
 @BasePares NVARCHAR(20) -- Nombre de la base de pares a crear
AS
BEGIN
 ...
 BEGIN TRANSACTION;
 		...
 		-- 1. Base de viajes alternativos | _2_1_base_viajes_tc_1
		SET @sql =
		'SELECT * 
		INTO '+QUOTENAME(@database)+'.dbo._2_1_base_viajes_tc_1
		FROM '+QUOTENAME(@database)+'.[dbo]._2_base_viajes v
		WHERE v.TieneCombinacion = 1';
		EXEC sp_executesql @sql;

		-- 2. Calculo agrupado de viajes por ParOD | _2_1_agrupado_ParOD_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@database)+'.dbo._2_1_agrupado_ParOD_tc_1;
		SELECT 
			v.ParOD, v.distancia,
			v.IdO, v.IdD,
			v.z_origen,v.z_destino,
			...
			AVG(CAST(v.distance_h3 AS FLOAT)) AS Promediodistance_h3,
			AVG(CAST(v.distance_osm_drive AS FLOAT)) AS Promediodistance_osm_drive
		INTO '+QUOTENAME(@database)+'.dbo._2_1_agrupado_ParOD_tc_1
		FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v
		GROUP BY
			v.ParOD, v.IdO, v.IdD, v.z_origen,v.z_destino
		ORDER BY
			v.ParOD;';
		...
		-- 3. Calculo agrupado de viajes por ParOD y horario | _2_1_agrupado_ParOD_horario_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_agrupado_ParOD_horario_tc_1;
		SELECT 
			v.ParOD, v.distancia,
			v.IdO, v.IdD,
			v.z_origen,v.z_destino,
			...
			AVG(CAST(v.distance_h3 AS FLOAT)) AS Promediodistance_h3,
			AVG(CAST(v.distance_osm_drive AS FLOAT)) AS Promediodistance_osm_drive
		INTO '+QUOTENAME(@Database)+'.[dbo]._2_1_agrupado_ParOD_horario_tc_1
		FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v
		GROUP BY
			v.ParOD, v.distancia,
			v.IdO, v.IdD,
		...
		-- 4. Calculo agrupado de viajes por Distancia | _2_1_agrupado_distancia_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_agrupado_distancia_tc_1;
		SELECT 
			v.distancia,
			-- COLECTIVO
			SUM(CASE WHEN v.ModoMultimodal = ''A-COLECTIVO'' THEN v.ViajesExpandidos ELSE 0 END) AS v_colectivo_tc_1,
			...
			AVG(CAST(v.distance_h3 AS FLOAT)) AS Promediodistance_h3,
			AVG(CAST(v.distance_osm_drive AS FLOAT)) AS Promediodistance_osm_drive
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_agrupado_distancia_tc_1
		FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v
		GROUP BY
		 v.distancia,
		ORDER BY
		...
		-- 5. Calculo agrupado de viajes por Hora | _2_1_agrupado_hora_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_agrupado_hora_tc_1;
		SELECT 
			v.hora,
			-- COLECTIVO
			SUM(CASE WHEN v.ModoMultimodal = ''A-COLECTIVO'' THEN v.ViajesExpandidos ELSE 0 END) AS v_colectivo_tc_1,
			...
			AVG(CAST(v.distance_h3 AS FLOAT)) AS Promediodistance_h3,
			AVG(CAST(v.distance_osm_drive AS FLOAT)) AS Promediodistance_osm_drive
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_agrupado_hora_tc_1
		FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v
		GROUP BY
		 v.hora,
		ORDER BY
		...
		-- 6. Calculo agrupado de viajes por Pico Horario | _2_1_agrupado_Pico_Horario_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_agrupado_Pico_Horario_tc_1;
		SELECT 
			v.pico_horario,
			-- COLECTIVO
			SUM(CASE WHEN v.ModoMultimodal = ''A-COLECTIVO'' THEN v.ViajesExpandidos ELSE 0 END) AS v_colectivo_tc_1,
			...
			AVG(CAST(v.distance_h3 AS FLOAT)) AS Promediodistance_h3,
			AVG(CAST(v.distance_osm_drive AS FLOAT)) AS Promediodistance_osm_drive
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_agrupado_Pico_Horario_tc_1
		FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v
		GROUP BY
			v.pico_horario,
		ORDER BY
		...
		-- 7. Calculo distribución de viajes por ModoMultimodal y Combinacion | _2_1_distribucion_MultiModal_ModoCombinacion_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_distribucion_MultiModal_ModoCombinacion_tc_1;
		SELECT 
			v.ModoMultimodal, v.ModoCombinacion,v.TieneCombinacion,
			SUM(v.ViajesExpandidos) AS v_totales,
			CASE WHEN (SELECT SUM(v.ViajesExpandidos) FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v) > 0 
			 THEN SUM(v.ViajesExpandidos) / (SELECT SUM(v.ViajesExpandidos) FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v)
			 ELSE 0 
			END AS Porcentaje,
			COUNT(DISTINCT v.id_tarjeta) AS TarjetasUnicas,
			SUM(CAST(v.tren AS INT)) AS tren,
			...
			AVG(CAST(v.distance_h3 AS FLOAT)) AS PromedioDistance_h3,
			AVG(CAST(v.distance_osm_drive AS FLOAT)) AS PromedioDistance_osm_drive
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_distribucion_MultiModal_ModoCombinacion_tc_1
		FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v
		WHERE v.TieneCombinacion = 0
		GROUP BY
			v.ModoMultimodal, v.ModoCombinacion,v.TieneCombinacion
		...
		-- 8. Calculo distribución de viajes por Distancia y ModoMultimodal | _2_1_distribucion_distancia_MultiModal_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_distribucion_distancia_MultiModal_tc_1;
		SELECT 
			v.distancia, v.ModoMultimodal,v.TieneCombinacion,
			SUM(v.ViajesExpandidos) AS v_totales,
			CASE WHEN (SELECT SUM(v.ViajesExpandidos) FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v) > 0 
			...
			AVG(CAST(v.distance_h3 AS FLOAT)) AS PromedioDistance_h3,
			AVG(CAST(v.distance_osm_drive AS FLOAT)) AS PromedioDistance_osm_drive
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_distribucion_distancia_MultiModal_tc_1
		 FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v
		GROUP BY
			v.distancia, v.ModoMultimodal,v.TieneCombinacion
		ORDER BY
		...
		-- 9. Calculo distribución de viajes por ModoMultimodal, Combinacion y horario | _2_1_distribucion_distancia_ModoMultiModal_Combinacion_horario_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_distribucion_distancia_ModoMultiModal_Combinacion_horario_tc_1;
		SELECT 
			v.ModoMultimodal,v.ModoCombinacion,v.pico_horario,v.TieneCombinacion,
			SUM(v.ViajesExpandidos) AS v_totales,
			CASE WHEN (SELECT SUM(v.ViajesExpandidos) FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v) > 0 
			...
			AVG(CAST(v.distance_h3 AS FLOAT)) AS PromedioDistance_h3,
			AVG(CAST(v.distance_osm_drive AS FLOAT)) AS PromedioDistance_osm_drive
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_distribucion_distancia_ModoMultiModal_Combinacion_horario_tc_1
		 FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v
		GROUP BY
			v.ModoMultimodal,v.ModoCombinacion,v.pico_horario,v.TieneCombinacion
		ORDER BY
		...
		-- 10. Calculo distribución de viajes por ModoMultimodal, Combinacion, distancia y horario | _2_1_distribucion_distancia_ModoMultiModal_Combinacion_distancia_horario_0
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_distribucion_distancia_ModoMultiModal_Combinacion_distancia_horario_0;
		SELECT 
			v.ModoMultimodal,v.ModoCombinacion,v.distancia,v.pico_horario,v.TieneCombinacion,
			SUM(v.ViajesExpandidos) AS v_totales,
			CASE WHEN (SELECT SUM(v.ViajesExpandidos) FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v) > 0 
			...
			AVG(CAST(v.distance_h3 AS FLOAT)) AS PromedioDistance_h3,
			AVG(CAST(v.distance_osm_drive AS FLOAT)) AS PromedioDistance_osm_drive
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_distribucion_distancia_ModoMultiModal_Combinacion_distancia_horario_0
		 FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v
		GROUP BY
			v.ModoMultimodal,v.ModoCombinacion,v.distancia,v.pico_horario,v.TieneCombinacion
		ORDER BY
		...
		-- 11. Calculo de distribución de viajes por Horario y distancia | _2_1_distribucion_horario_distancia_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_distribucion_horario_distancia_tc_1;
		SELECT 
			v.pico_horario, v.distancia, v.TieneCombinacion,
			SUM(v.ViajesExpandidos) AS v_totales,
			CASE WHEN (SELECT SUM(v.ViajesExpandidos) FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v) > 0 
			...
			AVG(CAST(v.distance_h3 AS FLOAT)) AS PromedioDistance_h3,
			AVG(CAST(v.distance_osm_drive AS FLOAT)) AS PromedioDistance_osm_drive
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_distribucion_horario_distancia_tc_1
		FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v
		GROUP BY
			v.pico_horario, v.distancia, v.TieneCombinacion
		ORDER BY
		...
		-- 12. Calculo de distribución de viajes por horario y modo de combinacion | _2_1_distribucion_horario_ModoCombinacion_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_distribucion_horario_ModoCombinacion_tc_1;
		SELECT 
			v.pico_horario, v.ModoCombinacion, v.TieneCombinacion,
			SUM(v.ViajesExpandidos) AS v_totales,
			CASE WHEN (SELECT SUM(v.ViajesExpandidos) FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v) > 0 
			...
			AVG(CAST(v.distance_h3 AS FLOAT)) AS PromedioDistance_h3,
			AVG(CAST(v.distance_osm_drive AS FLOAT)) AS PromedioDistance_osm_drive
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_distribucion_horario_ModoCombinacion_tc_1
		FROM '+QUOTENAME(@database)+'.[dbo]._2_1_base_viajes_tc_1 v
		GROUP BY
			v.pico_horario, v.ModoCombinacion, v.TieneCombinacion
		ORDER BY
		...
		-- 13. Calculo de etapas de cada linea por ParOD y Modo Multimodal | _2_1_2_etapas_por_linea_porParOD_ModoMultimodal_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_2_etapas_por_linea_porParOD_ModoMultimodal_tc_1;
		SELECT 
			v.ParOD, v.distancia,v.IdO,v.IdD, v.z_origen,v.z_destino, v.ModoMultimodal, v.id_linea, v.nombre_linea, v.empresa,
			COUNT (v.id_linea) AS CantidadEtapas
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_2_etapas_por_linea_porParOD_ModoMultimodal_tc_1
		FROM '+QUOTENAME(@Database)+'.dbo._2_1_base_combinaciones_por_linea_porParOD_TC_1 v
		GROUP BY
			v.ParOD, v.IdO,v.IdD, v.z_origen,v.z_destino, v.distancia, v.ModoMultimodal, v.id_linea, v.nombre_linea,	v.empresa
		ORDER BY
			v.ParOD,v.distancia,v.ModoMultimodal;';
		...
		-- 14. Calculo de etapas de cada linea por ParOD | _2_1_2_etapas_por_linea_porParOD_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_2_etapas_por_linea_porParOD_tc_1;
		SELECT 
			v.ParOD, v.distancia,v.IdO,v.IdD, v.z_origen,v.z_destino, v.id_linea, v.nombre_linea, v.empresa,
			COUNT (v.id_linea) AS CantidadEtapas
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_2_etapas_por_linea_porParOD_tc_1
		FROM '+QUOTENAME(@Database)+'.dbo._2_1_base_combinaciones_por_linea_porParOD_TC_1 v
		GROUP BY
			v.ParOD, v.IdO,v.IdD, v.z_origen,v.z_destino, v.distancia, v.id_linea, v.nombre_linea,	v.empresa
		ORDER BY
		...
		-- 15. Calculo de etapas de cada linea por Distancia | _2_1_2_etapas_por_linea_por_distancia_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_2_etapas_por_linea_por_distancia_tc_1;
		SELECT 
			v.distancia,v.IdO,v.IdD, v.z_origen,v.z_destino, v.id_linea, v.nombre_linea, v.empresa,
			COUNT (v.id_linea) AS CantidadEtapas
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_2_etapas_por_linea_por_distancia_tc_1
		FROM '+QUOTENAME(@Database)+'.dbo._2_1_base_combinaciones_por_linea_porParOD_TC_1 v
		GROUP BY
			v.distancia, v.IdO,v.IdD, v.z_origen,v.z_destino, v.id_linea, v.nombre_linea,	v.empresa
		ORDER BY
		...
		-- 16. Calculo de etapas de cada linea por ModoMultimodal | _2_1_2_etapas_por_linea_por_ModoMultimodal_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_2_etapas_por_linea_por_ModoMultimodal_tc_1;
		SELECT 
			v.ModoMultimodal,v.IdO,v.IdD, v.z_origen,v.z_destino, v.id_linea, v.nombre_linea, v.empresa,
			COUNT (v.id_linea) AS CantidadEtapas
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_2_etapas_por_linea_por_ModoMultimodal_tc_1
		FROM '+QUOTENAME(@Database)+'.dbo._2_1_base_combinaciones_por_linea_porParOD_TC_1 v
		GROUP BY
			v.ModoMultimodal, v.IdO,v.IdD, v.z_origen,v.z_destino, v.id_linea, v.nombre_linea,	v.empresa
		ORDER BY
		...
		-- 17. Combinacion de viaje por ParOD | _2_1_1_combinacion_mas_utilizadas_porParOD_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_mas_utilizadas_porParOD_tc_1;
		SELECT 
			ParOD, distancia,z_origen, z_destino, NombreCombinacion, CantidadRepeticiones, SumaViajes,
			PARTITIONED.NombreCombinacion_Part1 AS Etapa1,
			PARTITIONED2.NombreCombinacion_Part2 AS Etapa2,
			...
			PARTITIONED9.NombreCombinacion_Part9 AS Etapa9,
			ROW_NUMBER() OVER (PARTITION BY ParOD ORDER BY SumaViajes DESC) AS NroFila
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_mas_utilizadas_porParOD_tc_1
		FROM (
			SELECT 
				ParOD, distancia, z_origen, z_destino, NombreCombinacion, CantidadRepeticiones,
				SUM(SumaViajes) AS SumaViajes, -- Primero calculo la suma de etapas por par OD,
				ROW_NUMBER() OVER (PARTITION BY ParOD ORDER BY SUM(SumaViajes) DESC) AS NroFila -- Rankeo
			FROM '+QUOTENAME(@Database)+'.dbo._2_1_base_combinaciones_por_linea_porParOD_TC_1
			GROUP BY
				ParOD, z_origen, z_destino, distancia,NombreCombinacion, CantidadRepeticiones
		) AS Subquery
		OUTER APPLY (
		...
		-- 18. Combinacion de viaje por ParOD y Modo | _2_1_1_combinacion_utilizadas_ParOD_Modo_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_ParOD_Modo_tc_1;
		SELECT 
			ParOD, z_origen, z_destino, NombreCombinacion, distancia, ModoMultimodal, CantidadRepeticiones, SumaViajes,
			ROW_NUMBER() OVER (PARTITION BY ParOD, ModoMultimodal ORDER BY SumaViajes DESC) AS NroFila, -- Rankeo
			PARTITIONED.NombreCombinacion_Part1 AS Etapa1,
			...
			PARTITIONED8.NombreCombinacion_Part8 AS Etapa8,
			PARTITIONED9.NombreCombinacion_Part9 AS Etapa9
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_ParOD_Modo_tc_1 -- Lineas Más Utilizadas ParOD y Modo
		FROM (
			SELECT 
				ParOD, z_origen, z_destino, NombreCombinacion, distancia, CantidadRepeticiones, ModoMultimodal,
				SUM(SumaViajes) AS SumaViajes, 
		...
		-- 19. Combinacion de viaje por ParOD y Horario| _2_1_1_combinacion_utilizadas_ParOD_Horario_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_ParOD_Horario_tc_1;
		SELECT 
			ParOD,distancia z_origen, z_destino,NombreCombinacion, distancia, pico_horario, CantidadRepeticiones, SumaViajes,
			ROW_NUMBER() OVER (PARTITION BY ParOD, pico_horario ORDER BY SumaViajes DESC) AS NroFila, -- Rankeo
			PARTITIONED.NombreCombinacion_Part1 AS Etapa1,
			...
			PARTITIONED8.NombreCombinacion_Part8 AS Etapa8,
			PARTITIONED9.NombreCombinacion_Part9 AS Etapa9
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_ParOD_Horario_tc_1 -- Lineas Más Utilizadas ParOD y Modo
		FROM (
			SELECT 
				ParOD, z_origen, z_destino,NombreCombinacion, distancia, pico_horario, CantidadRepeticiones,
				SUM(SumaViajes) AS SumaViajes, -- Primero calculo la suma de etapas por par OD
		...
		-- 20. Combinacion de viaje por ParOD, Modo y Distancia | _2_1_1_combinacion_utilizadas_ParOD_Modo_Distancia_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_ParOD_Modo_Distancia_tc_1;
		SELECT 
			ParOD, distancia, z_origen, z_destino,NombreCombinacion, ModoMultimodal, CantidadRepeticiones, SumaViajes,
			ROW_NUMBER() OVER (PARTITION BY ParOD, ModoMultimodal, distancia ORDER BY SumaViajes DESC) AS NroFila, -- Rankeo
			PARTITIONED.NombreCombinacion_Part1 AS Etapa1,
			...
			PARTITIONED8.NombreCombinacion_Part8 AS Etapa8,
			PARTITIONED9.NombreCombinacion_Part9 AS Etapa9
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_ParOD_Modo_Distancia_tc_1 -- Lineas Más Utilizadas ParOD y Modo
		FROM (
			SELECT 
				ParOD, z_origen, z_destino, NombreCombinacion,distancia, ModoMultimodal, CantidadRepeticiones,
				SUM(SumaViajes) AS SumaViajes, -- Primero calculo la suma de etapas por par OD
		...
		-- 21. Combinacion de viaje por ParOd, Modo, Distancia y Modo | _2_1_1_combinacion_utilizadas_ParOD_Modo_Distancia_horario_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_ParOD_Modo_Distancia_horario_tc_1;
		SELECT 
			ParOD,distancia, z_origen, z_destino,NombreCombinacion, ModoMultimodal, pico_horario, CantidadRepeticiones,SumaViajes,
			ROW_NUMBER() OVER (PARTITION BY ParOD, ModoMultimodal, distancia, pico_horario ORDER BY SumaViajes DESC) AS NroFila, -- Rankeo
			PARTITIONED.NombreCombinacion_Part1 AS Etapa1,
			...
			PARTITIONED8.NombreCombinacion_Part8 AS Etapa8,
			PARTITIONED9.NombreCombinacion_Part9 AS Etapa9
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_ParOD_Modo_Distancia_horario_tc_1 -- Lineas Más Utilizadas ParOD y Modo
		FROM (
			SELECT 
				ParOD, z_origen, z_destino,NombreCombinacion, distancia, ModoMultimodal, pico_horario, CantidadRepeticiones,
				SUM(SumaViajes) AS SumaViajes, -- Primero calculo la suma de etapas por par OD
		...
		-- 22. Combinacion de viaje por Distancia | _2_1_1_combinacion_utilizadas_Distancia_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_Distancia_tc_1;
		SELECT 
			distancia, 
			NombreCombinacion,
			SUM(CantidadRepeticiones) AS CantidadRepeticiones,
			...
			PARTITIONED8.NombreCombinacion_Part8 AS Etapa8,
			PARTITIONED9.NombreCombinacion_Part9 AS Etapa9
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_Distancia_tc_1
		FROM (
			SELECT 
				distancia, 
				NombreCombinacion,
		...
		-- 23. Combinacion de viaje por Modo Multimodal | _2_1_1_combinacion_utilizadas_ModoMultimodal_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_ModoMultimodal_tc_1;
		SELECT 
			ModoMultimodal,NombreCombinacion,
			SUM(CantidadRepeticiones) AS CantidadRepeticiones,
			SUM(SumaViajes) AS SumaViajes,
			...
			PARTITIONED8.NombreCombinacion_Part8 AS Etapa8,
			PARTITIONED9.NombreCombinacion_Part9 AS Etapa9
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_ModoMultimodal_tc_1 -- Lineas Más Utilizadas ParOD y Modo
		FROM (
			SELECT 
				ModoMultimodal,NombreCombinacion,
				SUM(CantidadRepeticiones) AS CantidadRepeticiones,
		...
		-- 24. Combinacion de viaje por Horario | _2_1_1_combinacion_utilizadas_horario_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_horario_tc_1;
		SELECT 
			pico_horario, NombreCombinacion, 
			SUM(CantidadRepeticiones) AS CantidadRepeticiones,
			SUM(SumaViajes) AS TotalViajes,
			...
			PARTITIONED8.NombreCombinacion_Part8 AS Etapa8,
			PARTITIONED9.NombreCombinacion_Part9 AS Etapa9
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_horario_tc_1 -- Lineas Más Utilizadas ParOD y Modo
		FROM (
			SELECT 
				pico_horario,NombreCombinacion,
				SUM(CantidadRepeticiones) AS CantidadRepeticiones,
		...
		-- 25. Combinacion de viaje por Distancia y Modo | _2_1_1_combinacion_utilizadas_Distancia_Modo_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_Distancia_Modo_tc_1;
		SELECT 
			Distancia, ModoMultimodal, NombreCombinacion, 
			SUM(CantidadRepeticiones) AS CantidadRepeticiones,
			SUM(SumaViajes) AS TotalViajes,
			...
			PARTITIONED8.NombreCombinacion_Part8 AS Etapa8,
			PARTITIONED9.NombreCombinacion_Part9 AS Etapa9
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_Distancia_Modo_tc_1 -- Lineas Más Utilizadas ParOD y Modo
		FROM (
			SELECT 
				Distancia, ModoMultimodal,NombreCombinacion,
				SUM(CantidadRepeticiones) AS CantidadRepeticiones,
		...
		-- 26. Combinacion de viaje por Distancia, Modo y Horario | _2_1_1_combinacion_utilizadas_Distancia_Modo_Horario_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_Distancia_Modo_Horario_tc_1;
		SELECT 
			Distancia, pico_horario, ModoMultimodal,NombreCombinacion,
			SUM(CantidadRepeticiones) AS CantidadRepeticiones,
			SUM(SumaViajes) AS TotalViajes,
			...
			PARTITIONED8.NombreCombinacion_Part8 AS Etapa8,
			PARTITIONED9.NombreCombinacion_Part9 AS Etapa9
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_1_combinacion_utilizadas_Distancia_Modo_Horario_tc_1 -- Lineas Más Utilizadas ParOD y Modo
		FROM (
			SELECT 
				Distancia,pico_horario, ModoMultimodal,NombreCombinacion,
				SUM(CantidadRepeticiones) AS CantidadRepeticiones,
		...
		-- 27. Linea mas utilizada por ModoMultimodal | _2_1_2_top_linea_mas_utilizadas_ModoMultimodal_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_2_top_por_linea_por_ModoMultimodal_tc_1;
		SELECT 
			ModoMultimodal, id_linea, nombre_linea, empresa, CantidadEtapas,
			ROW_NUMBER() OVER (PARTITION BY ModoMultimodal ORDER BY CantidadEtapas DESC) AS NroFila
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_2_top_linea_mas_utilizadas_ModoMultimodal_tc_1 
		FROM (
			SELECT 
				ModoMultimodal, id_linea, nombre_linea, empresa,
				SUM(CantidadEtapas) AS CantidadEtapas,
		...
		-- 28. Linea mas utilizada por Distancia | _2_1_2_top_linea_mas_utilizadas_Distancia_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_2_top_linea_mas_utilizadas_Distancia_tc_1;
		SELECT 
			distancia, id_linea, nombre_linea, empresa, CantidadEtapas,
			ROW_NUMBER() OVER (PARTITION BY distancia ORDER BY CantidadEtapas DESC) AS NroFila -- Rankeo
		...
		-- 29. Linea mas utilizada por Pico Horario | _2_1_2_top_linea_mas_utilizadas_pico_horario_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_2_top_linea_mas_utilizadas_pico_horario_tc_1;
		SELECT 
			pico_horario, id_linea, nombre_linea, empresa, CantidadEtapas,
			ROW_NUMBER() OVER (PARTITION BY pico_horario ORDER BY CantidadEtapas DESC) AS NroFila
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_2_top_linea_mas_utilizadas_horario_tc_1
		FROM (
			SELECT 
				pico_horario, id_linea, nombre_linea, empresa,
				SUM(CantidadEtapas) AS CantidadEtapas,
		...
		-- 30. Linea mas utilizada por Hora | _2_1_2_top_linea_mas_utilizadas_hora_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_2_top_linea_mas_utilizadas_hora_tc_1;
		SELECT 
			hora, id_linea, nombre_linea, empresa, CantidadEtapas,
			ROW_NUMBER() OVER (PARTITION BY hora ORDER BY CantidadEtapas DESC) AS NroFila
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_2_top_linea_mas_utilizadas_hora_tc_1
		FROM (
			SELECT 
				hora, id_linea, nombre_linea, empresa,
				SUM(CantidadEtapas) AS CantidadEtapas,
		...
		-- 31. Linea mas utilizada por Distancia Horario | _2_1_2_top_linea_mas_utilizadas_Distancia_horario_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_2_top_linea_mas_utilizadas_Distancia_horario_tc_1;
		SELECT 
			distancia, pico_horario, id_linea, nombre_linea, empresa, CantidadEtapas,
			ROW_NUMBER() OVER (PARTITION BY distancia, pico_horario ORDER BY CantidadEtapas DESC) AS NroFila -- Rankeo
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_2_top_linea_mas_utilizadas_Distancia_horario_tc_1;
		FROM (
			SELECT 
				distancia, pico_horario, id_linea, nombre_linea, empresa,
				SUM(CantidadEtapas) AS CantidadEtapas, 
		...
		-- 32. Linea mas utilizada por ModoMultimodal y PicoHorario | _2_1_2_top_linea_mas_utilizadas_ModoMultimodal_PicoHorario_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_2_top_linea_mas_utilizadas_ModoMultimodal_PicoHorario_tc_1;
		SELECT 
			ModoMultimodal, pico_horario, id_linea, nombre_linea, empresa, CantidadEtapas,
			ROW_NUMBER() OVER (PARTITION BY ModoMultimodal, pico_horario ORDER BY CantidadEtapas DESC) AS NroFila -- Rankeo
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_2_top_linea_mas_utilizadas_ModoMultimodal_PicoHorario_tc_1;
		FROM (
			SELECT 
				ModoMultimodal, pico_horario, id_linea, nombre_linea, empresa,
				SUM(CantidadEtapas) AS CantidadEtapas, 
		...
		-- 33. Linea mas utilizada por ModoMultimodal y Distancia | _2_1_2_top_linea_mas_utilizadas_ModoMultimodal_Distancia_tc_1
		SET @sql =
		'DROP TABLE IF EXISTS '+QUOTENAME(@Database)+'.dbo._2_1_2_top_linea_mas_utilizadas_ModoMultimodal_Distancia_tc_1;
		SELECT 
			ModoMultimodal, distancia, id_linea, nombre_linea, empresa, CantidadEtapas,
			ROW_NUMBER() OVER (PARTITION BY ModoMultimodal, distancia ORDER BY CantidadEtapas DESC) AS NroFila -- Rankeo
		INTO '+QUOTENAME(@Database)+'.dbo._2_1_2_top_linea_mas_utilizadas_ModoMultimodal_Distancia_tc_1;
		FROM (
			SELECT 
				ModoMultimodal, distancia, id_linea, nombre_linea, empresa,
				SUM(CantidadEtapas) AS CantidadEtapas, 
